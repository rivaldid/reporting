...
CREATE VIEW `VARCHI` AS
SELECT CONCAT_WS(' ','PULSAR',centrale,varco) sensore,label FROM SER_VARCHI ORDER BY sensore;

CREATE VIEW `SERCHIO_OSPITI` AS
SELECT DISTINCT(Ospite) FROM SERCHIO WHERE Data>=(CURDATE()-INTERVAL 1 MONTH) ORDER BY Ospite;
