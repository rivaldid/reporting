...
CREATE TABLE `WIN_REPORT` (
  ...
  PRIMARY KEY (`Wid`),
  CONSTRAINT FOREIGN KEY (`id_evento`) REFERENCES WIN_EVENTI(`id_evento`),
  CONSTRAINT FOREIGN KEY (`id_messaggio`) REFERENCES WIN_MESSAGGI(`id_messaggio`),
  CONSTRAINT FOREIGN KEY (`Rid`) REFERENCES REPOSITORY(`Rid`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `SER_REPORT` (
  ...
  PRIMARY KEY (`Sid`),
  CONSTRAINT FOREIGN KEY (`id_tessera`) REFERENCES SER_TESSERE(`id_tessera`),
  CONSTRAINT FOREIGN KEY (`id_evento`) REFERENCES SER_EVENTI(`id_evento`),
  CONSTRAINT FOREIGN KEY (`id_varco`) REFERENCES SER_VARCHI(`id_varco`),
  CONSTRAINT FOREIGN KEY (`id_ospite`) REFERENCES SER_OSPITI(`id_ospite`),
  CONSTRAINT FOREIGN KEY (`Rid`) REFERENCES REPOSITORY(`Rid`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `ADC_REPORT` (
  ...
  `data_report` date DEFAULT NULL,
  `Rid` int(11) DEFAULT NULL,
  PRIMARY KEY (`Aid`),
  CONSTRAINT FOREIGN KEY (`id_ospite`) REFERENCES ADC_OSPITI(`id_ospite`),
  CONSTRAINT FOREIGN KEY (`id_documento`) REFERENCES ADC_DOCUMENTI(`id_documento`),
  CONSTRAINT FOREIGN KEY (`id_struttura`) REFERENCES ADC_STRUTTURE(`id_struttura`),
  CONSTRAINT FOREIGN KEY (`id_profilo`) REFERENCES ADC_PROFILI(`id_profilo`),
  CONSTRAINT FOREIGN KEY (`Rid`) REFERENCES REPOSITORY(`Rid`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;